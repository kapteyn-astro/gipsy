...
@integer function lsqfitd( double precision, integer, double precision,
@   double precision, integer, double precision, double precision,
@   integer, integer, double precision, integer, double precision, integer )
...
extern double funcd_c( double *xdat ,		/* returns function value */
                       double *fpar ,
                       fint   *npar ,
                       fint   *fopt );
extern void  dervd_c( double *xdat ,		/* returns derivatives */
                      double *fpar ,
                      double *epar ,
                      fint   *npar ,
                      fint   *fopt );
...
static void getmat( double *xdat ,
                    fint   *xdim ,
                    double *ydat ,
                    double *wdat ,
                    fint   *ndat ,
 ...
 */
{
   ...
   for (n = 0; n < (*ndat); n++) {		/* loop trough data points */
      ...
      if (wn > 0.0) {				/* legal weight ? */
         yd = ydat[n] - funcd_c( &xdat[(*xdim) * n], fpar, npar, fopt );
         dervd_c( &xdat[(*xdim) * n], fpar, epar, npar, fopt );
...
static double funcd_c( double *xdat, double *fpar, fint *npar, fint *fopt )
/*
 * if *fopt == 1:
...
static void dervd_c( double *xdat, double *fpar, double *epar, fint *npar, fint *fopt )
{
   if (*fopt == 1) {
      double a;
      double b;
...
void main( )
{
   ...
   for (nfit = 0; nfit < 10; nfit++) {
      ...
      for (n = 0; n < ndat; n++) {
         ...
         xdat[n] = (double) n;
         wdat[n] = 1.0;
         ydat[n] = funcd_c( &xdat[n], fpar, &npar, &opt );
         rndm = (double) ( rand( ) - RAND_MAX / 2 ) / (double) RAND_MAX * 1.0;
         ydat[n] += rndm;
      }
      for (m = 0; m < npar; m++) {
         double rndm;
         rndm = (double) ( rand( ) - RAND_MAX / 2 ) / (double) RAND_MAX * 3.0;
         fpar[n] += rndm;
      }
      printf( "lsqfitd:" );
      r = lsqfitd_c( xdat, &xdim, ydat, wdat, &ndat, fpar, epar, mpar, &npar, &tol, &its, &lab, &opt );
      printf( " %ld", r );
      if (r < 0) {
         printf( ", error!\n" );
      } else {
